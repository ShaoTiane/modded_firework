...
function createHeartBurst(particleFactory, centerX, centerY, spreadSize) {
	// 爱心参数方程生成的轮廓宽度约 32 单位，按 spreadSize 等比缩放
	const scale = spreadSize / 32;
	// 提高采样点数（沿轮廓密集采样，后续自适应稀释）
	const candidateSteps = Math.min(Math.floor(spreadSize * 0.9), 500);
	const color = randomColor();
	const strobed = Math.random() < 0.5;
	const strobeColor = strobed ? randomColor() : color;
	...
	for (let i = 0; i < candidateSteps; i++) {
		const t = (i / candidateSteps) * PI_2;
		const hx = 16 * Math.pow(Math.sin(t), 3);
		const hy = -(13 * Math.cos(t) - 5 * Math.cos(2 * t) - 2 * Math.cos(3 * t) - Math.cos(4 * t));

		// 计算曲线在该点的"行进速度"（弧长导数）
		// dx/dt = 48 * sin²(t) * cos(t)
		// dy/dt 类似（省略精确计算，用数值差分近似足够）
		const tNext = t + 0.001;
		const hxNext = 16 * Math.pow(Math.sin(tNext), 3);
		const hyNext = -(13 * Math.cos(tNext) - 5 * Math.cos(2 * tNext) - 2 * Math.cos(3 * tNext) - Math.cos(4 * tNext));
		const speed = Math.sqrt((hxNext - hx) * (hxNext - hx) + (hyNext - hy) * (hyNext - hy)) * 1000;

		candidates.push({ hx, hy, speed });
	}

	// 第二步：自适应稀释 —— 速度慢的地方（对称轴附近）跳过更多点
	// 找出最大速度用于归一化
	let maxSpeed = 0;
	for (let i = 0; i < candidates.length; i++) {
		if (candidates[i].speed > maxSpeed) maxSpeed = candidates[i].speed;
	}
	// 防止除零
	...
	for (let i = 0; i < candidates.length; i++) {
		...
		// 归一化速度作为保留概率：速度快→概率高，速度慢→概率低
		const keepProbability = Math.pow(c.speed / maxSpeed, 0.85);
		if (Math.random() > keepProbability) {
			continue; // 跳过密集区域的点
		}

const speedRatio = c.speed / maxSpeed;
const jitterAmount = 0.2 + 1.8 * Math.pow(speedRatio, 0.25);
const jitterX = (Math.random() - 0.5) * jitterAmount;
const jitterY = (Math.random() - 0.5) * jitterAmount;

		particleFactory(
			{
				x: centerX + (c.hx + jitterX) * scale,
				y: centerY + (c.hy + jitterY) * scale,
			},
			strobed ? strobeColor : color,
			strobed,
			strobeColor
		);
	}
...
class Shell {
	...
	burst(x, y) {
		...
		const dotStarFactory = (point, pointColor, strobe, strobeColor) => {
		    ...
		    if (this.heart) {
		    createHeartBurst(dotStarFactory, x, y, this.spreadSize);

...
function updateCustomWords(customText) {
    if (customText && customText.trim()) {
        // 用户输入了自定义文字，按逗号分割，过滤空字符串
        const words = customText.split(",").map(function(w) { return w.trim(); }).filter(function(w) { return w.length > 0; });
        randomWords.length = 0;
        randomWords.push.apply(randomWords, words);
    } else {
        // 用户没输入，恢复默认文字
        randomWords.length = 0;
        randomWords.push.apply(randomWords, appConfig.defaultWords);
    }
}
...
function configDidUpdate() {
	...
	updateCustomWords(store.state.config.wordText);
...
function handleBackgroundApply(rawValue) {
    const isBlobUrl = typeof rawValue === "string" && rawValue.startsWith("blob:");
    backgroundManager.applyBackground({ value: rawValue }).then((result) => {
        ...
        // blob URL 不持久化（刷新失效），存一个标记即可
        const storeValue = isBlobUrl ? "" : (result.settings.value || "");

        store.setState({
            background: {
                mode: result.settings.mode || "image",
                value: storeValue,

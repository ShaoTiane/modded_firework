...
(function initFireworksAppUI(global) {
	...
	function readConfigFromDom(nodes) {
		return {
			...
			wordText: nodes.wordTextInput.value,
		};
	}
	...
	function renderApp(state, nodes) {
		...
		nodes.backgroundFileName.textContent = state.background.configured && state.background.value ? "已设置背景" : "";
		nodes.wordTextInput.value = state.config.wordText || "";
	...
	function bindAppControls(options) {
		...
		nodes.wordTextInput.addEventListener("change", syncConfig);
		...
		nodes.backgroundFileBtn.addEventListener("click", () => {
    nodes.backgroundFileInput.click();
});

nodes.backgroundFileInput.addEventListener("change", () => {
    const file = nodes.backgroundFileInput.files[0];
    if (!file) return;
    // 显示文件名
    nodes.backgroundFileName.textContent = file.name;
    // 创建 blob URL 并应用
    const blobUrl = URL.createObjectURL(file);
    options.onBackgroundApply(blobUrl);
    // 后续刷新时 blob URL 失效，但不影响——用户重新选图即可
    // 同时清掉 file input 以便重复选同一文件也能触发 change
    nodes.backgroundFileInput.value = "";
});
